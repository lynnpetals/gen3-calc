...
function performCalculations() {
	...
	for (var i = 0; i < setOptions.length; i++) {
		if (setOptions[i].id && typeof setOptions[i].id !== "undefined") {
			setName = setOptions[i].id.substring(
				setOptions[i].id.indexOf("(") + 1,
				setOptions[i].id.lastIndexOf(")")
			);
			...
			if (selectedTiers.indexOf(setTier) !== -1) {
				...
				var damageResults = calculateMovesOfAttacker(
					gen,
					attacker,
					defender,
					field
				...
				for (var n = 0; n < 4; n++) {
					...
					if (maxDamage > highestDamage) {
						...
						data.push(
							attacker.moves[n].name.replace("Hidden Power", "HP")
						);
						data.push(minPercentage + " - " + maxPercentage + "%");
						data.push(minPixels + " - " + maxPixels + "px");
						data.push(
							attacker.moves[n].bp === 0
								? "nice move"
								: result.kochance(false).text ||
...
function getSelectedTiers() {
	var selectedTiers = $(".tiers input:checked")
		.map(function () {
			return this.id;
		})
		.get();
...
function constructDataTable() {
	table = $("#holder-2").DataTable({
		...
		columnDefs: [
			...
			{ targets: [4], iDataSort: 2 },
		],
		dom: 'C<"clear">fti',
		colVis: {
			exclude:
				gen > 2 ? [0, 1, 2] : gen === 2 ? [0, 1, 2, 7] : [0, 1, 2, 7, 8],
...
function placeBsBtn() {
	var honkalculator =
		"<button style='position:absolute' class='bs-btn bs-btn-default'>Honkalculate</button>";
...
$(".mode").change(function () {
	if ($("#one-vs-one").prop("checked")) {
		...
		window.location.replace(
			"index" + linkExtension + (params.length ? "?" + params : "")
		);
		...
		params = "" + params;
		window.location.replace(
			"randoms" + linkExtension + (params.length ? "?" + params : "")
		);
	} else {
		var params = new URLSearchParams(window.location.search);
...
$(".tiers label").mouseup(function () {
	...
	if (
		(startsWith(oldID, "VGC") || oldID === "LC") &&
		!startsWith(newID, "VGC") &&
		newID !== "LC"
	) {
		setLevel("100");
	}
...
$(".tiers input").change(function () {
	...
	$(".tiers input")
		.not(":" + type)
		.prop("checked", false); // deselect all radios if a checkbox is checked, and vice-versa
...
$(document).ready(function () {
	...
	$("#holder-2 th:first").text(
		mode === "one-vs-all" ? "Defender" : "Attacker"
	);
	$("#holder-2").show();
...
function calcDTDimensions() {
	...
	var heightUnderDT =
		getBottomOffset($(".holder-0")) - getBottomOffset($("#holder-2 tbody"));
